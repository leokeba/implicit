...
- `src/core/toolpath-preview/` — the sliced toolpath as it appears in the viewport: `build.ts` turns points into per-segment typed arrays and one colour channel per quantity (speed, flow ratio, layer height, feature type, any scene field a postprocess script sampled), `renderer.ts` draws them as instanced camera-facing bead ribbons with depth, `color-ramps.ts` defines the ramps once for both the shader's lookup texture and the DOM legend, `domain.ts` measures ramp ranges over whatever layer range is visible. `src/core/preview.ts` hosts the stacked canvases; `src/core/slicer/preview-points.ts` splices the brim and solid-bottom fill (generated inside the G-code builder) back into the previewed point stream.
...
- `src/core/bambu/` — Bambu `.gcode.3mf` packaging: `zip.ts` (minimal ZIP writer, deflate via `CompressionStream`), `md5.ts` (the sidecar hash the printer validates; WebCrypto has no MD5), `gcode-stats.ts` (filament/duration/layer counts re-read from the emitted G-code), `png.ts` + `thumbnail.ts` (plate preview rasterised from the G-code's XY extrusion moves — the WebGL viewport has no preserved drawing buffer to capture), `package.ts` (assembles the OPC parts). `connection-kind.ts` names the two send backends.
