...
export function transform(context: any) {
    ...
    const nextPoints = context.points.map((point: any, index: number) => {
        ...
        const offsetMm = signal * amplitudeMm;
    ...
    return {
        points: nextPoints,
        notes: [
            `Applied oscillating normal: amp=${amplitudeMm.toFixed(2)}mm source=${sourceModeLabel(sourceMode)} wave=${waveModeLabel(waveMode)} freq=${frequency.toFixed(2)} ${sourceMode < 0.5 ? 'cyc/100mm' : 'cyc/layer'}`,
        ],
    };
}
