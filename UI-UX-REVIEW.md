...
> **Update 2026-07-08: fixed.** The stack breakpoint moved from 1180px to 980px — the 981-1180 range now uses the narrow side-by-side grid (inspector 320-360px) that already existed in CSS but was preempted by the JS gate, so parameters (with their sliders) stay visible next to the model on small laptops. On phones the top bar now lays the four selectors out in a 2x2 grid, roughly halving header height before the viewport.
...
16. **Editor resize handle has no keyboard handler** while the inspector one supports Arrow/Home (`src/App.svelte:1357-1374`). Neither uses `role="separator"` + `aria-valuenow`. *Fixed 2026-07-08: both editor resizers (side-dock and bottom-dock) support Arrow/Home; aria-labels mention the keys. Svelte's a11y linter rejects `role="separator"` on buttons, so they stay labeled buttons.*
17. **No `prefers-reduced-motion` support**, and the raymarch loop renders continuously when idle — a11y and battery. Pause the render loop when nothing changes. *Fixed 2026-07-08: the renderer tracks a dirty flag (camera, uniforms, resize, theme, overlay changes) and static scenes render 0 frames when idle (verified: 0 draws/s idle, 1 draw per camera move; scenes reading uTime/uFrameModulo still animate at full rate, e.g. Lamp Shade Copy at 60/s). `prefers-reduced-motion` forces the static path, freezing ambient animation. The shared shader's decorative uTime "pulse" was removed — it forced every scene to render at 60fps forever for a 2% brightness wobble.*
...
20. **Locale mismatch in numerals**: number inputs render "0,2" (browser locale) while summary chips show "0.20 mm" — same value, two formats on one screen. *Fixed 2026-07-08: chip formatting uses `toLocaleString`, so chips and inputs agree in every locale.*
...
22. **"SCENE EDITOR" and "FOLDER SYNC" chips are styled like buttons** but are static text — they read as dead controls. Distinguish badges from buttons. *Fixed 2026-07-08: badges are now small, borderless, muted labels clearly distinct from buttons.*
